...
* **增量扫描缓存**：支持可选的 SQLite 数据库缓存，记录文件的 `size`、`mtime` 和 `hash`。在重复扫描时只处理被修改过的文件，避免多余的硬盘读写，速度可提升数十至数百倍（若指定了缓存文件但初始化失败，系统将报错并终止程序，以防无声失效）。
...
| `--cache-file` | - | 否 | `None` | 指定 SQLite 增量扫描缓存数据库位置（若指定但初始化失败，则报错并终止程序）。 |
